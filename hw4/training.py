    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            # ====== YOUR CODE: ======
            actual_num_epochs += 1
            train = self.train_epoch(dl_train, **kw, verbose=verbose)
            test = self.test_epoch(dl_test, **kw, verbose=verbose)
            train_acc += [(train.accuracy)]
            train_loss += [loss for loss in train.losses]
            test_loss += [loss for loss in test.losses]
            test_acc += [(test.accuracy)]
            curr_loss = sum(test.losses) / len(test.losses)
            if num_epochs >= 2:
                ...
                if early_stopping and epochs_without_improvement >= early_stopping:
                    ...
                    break
            if post_epoch_fn:
                post_epoch_fn(epoch, train, test, verbose)
...
class VAETrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        #generate image
        fake_image, modu, sig = self.model.forward(x)
        #compute loss
        loss, data_loss, y = self.loss_fn(x, fake_image, modu, sig)
        #step
