...
class Episode(object):
    ...
    def calc_qvals(self, gamma: float) -> List[float]:
        ...
        # ====== YOUR CODE: ======
        q_ = 0.0
        changed = self.experiences.copy()
        changed.reverse()
       
        for e in changed:
            q_ = gamma * q_ + e.reward
            qvals.append(q_)
        qvals.reverse()
        # ========================
        return qvals
...
class TrainBatch(object):
    ...
    def from_episodes(cls, episodes: Iterable[Episode], gamma=0.999):
        ...
        # ====== YOUR CODE: ======
        qv, s, totr, ac =  torch.LongTensor(), torch.FloatTensor(),  torch.FloatTensor(),\
                                                 torch.FloatTensor()
        for epi in episodes:
            exp_actions = torch.Tensor([exp.action for exp in epi.experiences])
            exp_states = torch.stack([exp.state for exp in epi.experiences], dim=0)
            totr = torch.cat((totr, torch.FloatTensor([epi.total_reward])), dim=0)
            qv = torch.cat((qv, torch.FloatTensor(epi.calc_qvals(gamma))), dim=0)
            s = torch.cat((s, exp_states), dim=0)
            ac = torch.cat((ac, exp_actions), dim=0)
        tb = TrainBatch(s, ac, qv, totr)
        # ========================
        return tb
    ...
    def episode_batch_generator(self) -> Iterator[Tuple[Episode]]:
        ...
        while True:
            ...
            while finished == False:
                e = agent.step()
                episode_experiences += [(e)]
                episode_reward += e.reward
                finished = e.is_done
            curr_batch += [(Episode(episode_reward, episode_experiences))]
            episode_experiences = []
            episode_reward = 0.0
